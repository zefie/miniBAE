...
new_fluid_synth(fluid_settings_t *settings)
{
  ...
  /* Initialize portamento time mode (default: auto / xg-gs start) */
...
/*   fluid_mutex_unlock(synth->busy); */
  ...
  /* Track PORTAMENTO_TIME_LSB for auto portamento mode */
  if (num == PORTAMENTO_TIME_LSB && val != 0
      && synth->portamento_time_mode == FLUID_PORTAMENTO_TIME_MODE_AUTO) {
                  ...
                  char *response, int *response_len, int *handled, int dryrun)
{
  ...
  /* Roland GS SysEx message? */
  if (data[0] == MIDI_SYSEX_MANUF_ROLAND && len >= 8)
  {
    /* GS DT1: F0 41 dev 42 12 [addrH addrM addrL] [data...] chksum F7 */
    if (data[2] == 0x42 && data[3] == 0x12)
    {
      uint32 addr = ((uint32)data[4] << 16) | ((uint32)data[5] << 8) | data[6];
      int i, len_data = len - 8;
      int checksum = 0;
      ...
      if (checksum != (uint8)data[len - 1]) {
        return FLUID_OK;
      }

      /* GS Mode Set: addr 40 00 7F */
      if (addr == 0x40007F && len_data == 1)
      {
        if (data[7] != 0 && data[7] != 0x7F) {
          return FLUID_OK;
        }
        if (handled) *handled = 1;
        if (!dryrun) {
          if (data[7] == 0) {
            /* GS mode on - keep current bank selection behavior */
          } else {
            /* GS mode off (XG/GM) */
          }
        ...
        if (synth->verbose) {
          FLUID_LOG(FLUID_INFO, "SysEx GS DT1: bank selection mode is now %s",
                    data[7] == 0 ? "GS" : "GM");
        }
        return FLUID_OK;
      }

      /* GS Rhythm Part: addr 40 1n 10 (n = part index 0-15) */
      if (((addr & 0xFFF000) == 0x400000) && ((addr & 0xFFF) == 0x010) && len_data == 1)
      {
        int part = (addr >> 8) & 0x0F;
        int chan;
        ...
        if (!dryrun) {
          int type;
          /* Map GS part to MIDI channel: parts 0-9 map to channels 10,0-8; parts 10-15 map to channels 9-14 */
          chan = part >= 10 ? part : (part == 0 ? 9 : part - 1);
          if (chan >= synth->midi_channels) return FLUID_OK;

          type = (data[7] == 0x00) ? 0 : 1; /* 0=melodic, 1=drum */

          if (synth->verbose) {
            FLUID_LOG(FLUID_INFO, "SysEx GS DT1: setting MIDI channel %d to %s",
                      chan, type ? "drum" : "melodic");
          }

          /* Issue 1579: The preset for the channel needs to be forcibly changed.
           * Re-select the preset with appropriate bank for drum/melodic */
          fluid_channel_cc(synth->channel[chan], ALL_CTRL_OFF, 0);
          fluid_channel_set_banknum(synth->channel[chan],
                                     type ? DRUM_INST_BANK : 0);
...
/*   fluid_mutex_unlock(synth->busy); */
  ...
  /* Setup portamento pitch glide if active */
  if (synth->fromkey_portamento != INVALID_NOTE) {
    fluid_voice_update_portamento(voice, synth->fromkey_portamento, voice->key);
  }

  /* Update channel's portamento tracking */
  voice->channel->fromkey_portamento = voice->key;

  /* Track PORTAMENTO_TIME_LSB for auto portamento mode detection */
  if (voice->channel->cc[PORTAMENTO_TIME_LSB] != 0 && synth->portamento_time_mode == FLUID_PORTAMENTO_TIME_MODE_AUTO) {
    synth->portamento_time_has_seen_lsb = 1;
  }
...
fluid_synth_unset_program(fluid_synth_t* synth, int chan)
{
  if ((chan >= 0) && (chan < synth->midi_channels)) {
    ...
    if (channel) {
      fluid_preset_notify(channel->preset, FLUID_PRESET_UNSELECTED, channel->channum);
      channel->preset = NULL;
    }
    return FLUID_OK;
  }
 ...
 *
 * Set the channel type (drum or melodic).
 * In fluidlite, drum channels are identified by bank number (128).
 * This function is provided for API compatibility.
 */
void
fluid_synth_set_channel_type(fluid_synth_t* synth, int chan, int type)
{
  (void)synth;
  (void)chan;
  (void)type;
 ...
 *
 * Enable/disable reverb on an effects group (-1 for all).
 * In fluidlite, reverb is disabled via settings at init time.
 * This function is provided for API compatibility.
 */
void
...
{
  (void)fx_group;
  if (synth) synth->with_reverb = (on != 0) ? 1 : 0;
}

/*
 * fluid_synth_chorus_on
 *
 * Enable/disable chorus on an effects group (-1 for all).
 * In fluidlite, chorus is disabled via settings at init time.
 * This function is provided for API compatibility.
 */
void
...
{
  (void)fx_group;
  if (synth) synth->with_chorus = (on != 0) ? 1 : 0;
}
...
/**
 * Set the global portamento time mode of the synthesizer.
 * @param synth FluidSynth instance
 * @param mode The portamento time mode (FLUID_PORTAMENTO_TIME_MODE_AUTO / XG_GS / LINEAR)
 * @return FLUID_OK on success, FLUID_FAILED otherwise
 */
int fluid_synth_set_portamento_time_mode(fluid_synth_t* synth, int mode)
{
  if (synth == NULL) return FLUID_FAILED;
  if (mode < 0 || mode >= FLUID_PORTAMENTO_TIME_MODE_LAST) return FLUID_FAILED;
...
/**
 * Get the global portamento time mode of the synthesizer.
 * @param synth FluidSynth instance
 * @param mode Location to store the portamento time mode
 * @return FLUID_OK on success, FLUID_FAILED otherwise
 */
int fluid_synth_get_portamento_time_mode(fluid_synth_t* synth, int* mode)
{
  if (synth == NULL || mode == NULL) return FLUID_FAILED;

  *mode = (int)synth->portamento_time_mode;
