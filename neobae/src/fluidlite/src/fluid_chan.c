...
fluid_channel_init(fluid_channel_t* chan)
{
  ...
  chan->nrpn_select = 0;
  chan->nrpn_active = 0;
  chan->fromkey_portamento = INVALID_NOTE;
  chan->portamento_ctrl_key = INVALID_NOTE;
}
...
fluid_channel_init_ctrl(fluid_channel_t* chan, int is_all_ctrl_off)
{
  ...
  if (!is_all_ctrl_off) {
    ...
    chan->modulation_depth_range = 50.0f; /* 50 cents (GM2 default for RPN 0x05) */
...
/*      printf("** bank select msb recieved: %d\n", value); */
  ...
  case DATA_ENTRY_MSB:
	...
	switch (chan->cc[RPN_LSB])
	{
	  ...
	  case RPN_MODULATION_DEPTH_RANGE:
	    {
	      int msb_value = value;
	      int lsb_value = chan->cc[DATA_ENTRY_LSB];
	      /* MSB = semitones, LSB = 1/128 semitones (cent fraction)
	       * Default per GM2: 0 semitones + 50 cents */
	      fluid_channel_set_modulation_depth_range(chan,
	          msb_value * 100.0f + lsb_value * 100.0f / 128.0f);
	      fluid_synth_modulate_voices(chan->synth, chan->channum, 1, MODULATION_MSB);
	    }
	    break;
	}
      }
  ...
  case NRPN_MSB:
    chan->cc[NRPN_LSB] = 0;
    chan->cc[DATA_ENTRY_MSB] = 0;
    chan->cc[DATA_ENTRY_LSB] = 0;
    chan->nrpn_select = 0;
    chan->nrpn_active = 1;
    break;
  ...
  case PORTAMENTO_SWITCH:
    /* Portamento switch: >= 64 enables portamento */
    if (value < 64) {
      chan->fromkey_portamento = INVALID_NOTE;
    }
    /* Fall through to allow modulator updates */
    fluid_synth_modulate_voices(chan->synth, chan->channum, 1, num);
    break;

  case PORTAMENTO_CTRL:
    /* Portamento Control CC 84: sets the source key for next portamento */
    chan->portamento_ctrl_key = value;
    chan->fromkey_portamento = value;
    break;
