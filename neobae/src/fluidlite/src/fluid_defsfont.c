...
static size_t ovRead(void* ptr, size_t size, size_t nmemb, void* datasource)
{
    ...
    if (vd == NULL || ptr == NULL || size == 0 || nmemb == 0 ||
        vd->data == NULL || vd->pos < 0 || vd->pos >= vd->datasize) {
        return 0;
    }
    ...
    available = (size_t)(vd->datasize - vd->pos);
    bytes = requested < available ? requested : available;
    bytes -= bytes % size;

    if (bytes > 0) {
        const char* src = vd->data + vd->pos;
        memcpy(ptr, src, bytes);
        vd->pos += (ogg_int64_t)bytes;
    }
...
static int ovSeek(void* datasource, ogg_int64_t offset, int whence)
{
    ...
    target = base + offset;
    if (target < 0 || target > vd->datasize) {
        return -1;
    }
...
{
    struct VorbisData* vd = (struct VorbisData*)datasource;
    if (vd == NULL || vd->pos < 0 || vd->pos > LONG_MAX) {
        return -1;
    }
    return (long)vd->pos;
}
...
#if SF3_SUPPORT
      /* Detect Ogg Vorbis compression via the sampletype flag (SF3 bit 4)
       * or by checking for the OggS magic bytes at the sample data offset.
       * Some SF3 generators don't set the flag, so the magic check is a
       * necessary fallback. */
      {
        int is_ogg = (sample->sampletype & FLUID_SAMPLETYPE_OGG_VORBIS) != 0;
        if (!is_ogg && sample->data != NULL
            && sample->start + 4 <= sample->end + 1) {
          const unsigned char *p = (const unsigned char *)sample->data
                                   + sample->start;
          if (p[0] == 'O' && p[1] == 'g' && p[2] == 'g' && p[3] == 'S') {
            is_ogg = 1;
            FLUID_LOG(FLUID_WARN,
                      "SF3 sample '%s': OggS magic detected at offset %u, "
                      "but OGG_VORBIS flag is not set in sampletype (0x%x). "
                      "Decoding anyway.",
                      sample->name, (unsigned)sample->start,
                      (unsigned)sample->sampletype);
          }
        }
...
#define BUFFER_SIZE 4096
        ...
        OggVorbis_File vf;
        int channels = 1;
        struct VorbisData vorbisData;

        vorbisData.pos  = 0;
        vorbisData.data = (char*)sample->data + sample->start;
        vorbisData.datasize = sample->end + 1 - sample->start;

        FLUID_LOG(FLUID_DBG, "SF3 unpack: name='%s' offset=%d size=%d",
                  sample->name, sample->start, vorbisData.datasize);

        if (vorbisData.datasize <= 0) {
          FLUID_LOG(FLUID_WARN, "SF3 unpack '%s': invalid size %d",
                    sample->name, vorbisData.datasize);
        } else if (ov_open_callbacks(&vorbisData, &vf, 0, 0, ovCallbacks) == 0) {
          vorbis_info *vi = ov_info(&vf, -1);
          channels = (vi != NULL) ? vi->channels : 1;
          FLUID_LOG(FLUID_DBG, "SF3 unpack '%s': ov_open ok, channels=%d rate=%ld",
                    sample->name, channels,
                    (long)((vi != NULL) ? vi->rate : 0));

          int bytes_read = 0;
          int section = 0;
          for (;;) {
            char *newbuf = realloc(sampledata, sampledata_size + BUFFER_SIZE);
            if (newbuf == NULL) {
              FLUID_LOG(FLUID_ERR,
                        "SF3 unpack '%s': realloc failed at %d bytes",
                        sample->name, sampledata_size + BUFFER_SIZE);
              FLUID_FREE(sampledata);
              sampledata = NULL;
              sampledata_size = 0;
              break;
            }
            sampledata = (short *)newbuf;
            bytes_read = ov_read(&vf, (char*)sampledata + sampledata_size,
            ...
            if (bytes_read > 0) {
              ...
              if (bytes_read < 0) {
                FLUID_LOG(FLUID_WARN,
                          "SF3 unpack '%s': ov_read error %d after %d bytes",
                          sample->name, bytes_read, sampledata_size);
              }
              if (sampledata_size > 0) {
                short *shrunk = (short *)realloc(sampledata, sampledata_size);
                if (shrunk != NULL) sampledata = shrunk;
              } else {
                FLUID_FREE(sampledata);
                sampledata = NULL;
              }
          ...
          FLUID_LOG(FLUID_WARN, "SF3 unpack '%s': ov_open_callbacks failed",
                    sample->name);
        }

        // convert interleaved byte count to per-channel frame count
        sampleframes = sampledata_size / (sizeof(short) * (channels > 0 ? channels : 1));
        FLUID_LOG(FLUID_DBG,
                  "SF3 unpack '%s': bytes=%d channels=%d frames=%d sampledata=%p",
                  sample->name, sampledata_size, channels, sampleframes,
                  (void*)sampledata);
#undef BUFFER_SIZE
#endif
...
#endif
        // if the ogg vorbis decode produced no data, mark the sample invalid
        // so the voice will not play it (otherwise voice->end == -1 and
        // the dsp code segfaults on dsp_data[voice->start]).
        if (sampledata == NULL || sampleframes <= 0) {
          FLUID_LOG(FLUID_WARN,
                    "SF3 '%s': decode failed, disabling sample",
                    sample->name);
          sample->valid = 0;
          sample->data = NULL;
          sample->start = 0;
      ...
      }     /* Ogg Vorbis detection block */
#else
      /* SF3 support not compiled in, but sample may have the OGG_VORBIS
       * flag set from the soundfont file.  Mark it invalid so the DSP
       * won't try to play raw compressed data as PCM. */
      if (sample->sampletype & FLUID_SAMPLETYPE_OGG_VORBIS) {
        FLUID_LOG(FLUID_WARN,
                  "SF3 sample '%s' found but SF3 support is not compiled in "
                  "(SF3_SUPPORT=%d), disabling sample",
                  sample->name, (int)SF3_SUPPORT);
        sample->valid = 0;
      }
#endif

      /* Post-check: if the sample still has the OGG_VORBIS flag after
       * the decode attempt (whether SF3 support is enabled or not),
       * something went wrong.  Disable the sample to prevent the DSP
       * from segfaulting on out-of-bounds data offsets. */
      if (sample->sampletype & FLUID_SAMPLETYPE_OGG_VORBIS) {
        FLUID_LOG(FLUID_WARN,
                  "SF3 sample '%s' was not properly decoded "
                  "(sampletype=0x%x, start=%u, end=%u), disabling",
                  sample->name, (unsigned)sample->sampletype,
                  (unsigned)sample->start, (unsigned)sample->end);
        sample->valid = 0;
      }
...
fluid_defpreset_noteon(fluid_defpreset_t* preset, fluid_synth_t* synth, int chan, int key, int vel)
{
  ...
  while (preset_zone != NULL) {
	...
	if (fluid_sample_in_rom(sample) || (sample == NULL) || !sample->valid) {
