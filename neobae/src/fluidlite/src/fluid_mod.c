...
// end S. Christian Collins' mod
  ...
  /* Apply modulation depth range scaling (RPN 0x05, GM2) for CC1 modulation
   * targeting VIBLFOTOPITCH or MODLFOTOPITCH */
  if ((mod->flags1 & FLUID_MOD_CC) && (mod->src1 == MODULATION_MSB)
      && ((mod->dest == GEN_VIBLFOTOPITCH) || (mod->dest == GEN_MODLFOTOPITCH))) {
    v1 *= fluid_channel_get_modulation_depth_range(chan) / 50.0f;
  }
