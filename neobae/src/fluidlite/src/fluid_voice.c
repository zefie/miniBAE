		 ...
		 unsigned int start_time, fluid_real_t gain)
{
  ...
  /* portamento */
  voice->portamento_active = 0;
  voice->portamento_pitch_offset = 0.0f;
  voice->portamento_pitch_incr = 0.0f;
		 ...
		 fluid_real_t* dsp_reverb_buf, fluid_real_t* dsp_chorus_buf)
{
  ...
  {
    FLUID_LOG(FLUID_WARN, "voice %p: invalid sample (sample=%p data=%p), turning off",
              (void*)voice, (void*)voice->sample,
              voice->sample ? (void*)voice->sample->data : NULL);
    fluid_voice_off(voice);
    return FLUID_OK;
  }

  /* Additional guard: if the sample bounds are invalid, don't play */
  if ((int)voice->sample->start < 0 || (int)voice->sample->end < (int)voice->sample->start)
  {
    FLUID_LOG(FLUID_WARN, "voice %p: invalid sample bounds start=%d end=%d, turning off",
              (void*)voice, (int)voice->sample->start, (int)voice->sample->end);
    fluid_voice_off(voice);
    return FLUID_OK;
  }
  ...
  voice->phase_incr = fluid_ct2hz_real
    (voice->pitch + voice->modlfo_val * voice->modlfo_to_pitch
     ...
     + voice->portamento_pitch_offset) / voice->root_pitch_hz;
...
#define FILTER_TRANSITION_SAMPLES (FLUID_BUFSIZE)
  ...
  /* Update portamento pitch offset towards 0 */
  if (voice->portamento_active) {
    if (fabs(voice->portamento_pitch_offset) <= fabs(voice->portamento_pitch_incr) * FLUID_BUFSIZE) {
      voice->portamento_pitch_offset = 0.0f;
      voice->portamento_active = 0;
      voice->portamento_pitch_incr = 0.0f;
    } else {
      voice->portamento_pitch_offset -= voice->portamento_pitch_incr * FLUID_BUFSIZE;
    }
  }
...
/* Compute the nominal pitch for a given key, accounting for SCALETUNE and root_key */
static fluid_real_t
fluid_voice_calculate_pitch(fluid_voice_t* voice, int key)
{
  fluid_real_t pitch;

  if (fluid_channel_has_tuning(voice->channel)) {
    fluid_tuning_t* tuning = fluid_channel_get_tuning(voice->channel);
    fluid_real_t x = fluid_tuning_get_pitch(tuning, (int)(voice->root_key / 100.0f));
...
/* Setup portamento pitch glide from fromkey to tokey */
void fluid_voice_update_portamento(fluid_voice_t *voice, int fromkey, int tokey)
{
  ...
  /* Calculate portamento time in ms */
  ms = fluid_channel_portamentotime_with_mode(channel, tm,
        channel->synth->portamento_time_has_seen_lsb, fromkey, tokey);

  /* Calculate increment count - number of buffers for the glide */
  countinc = (unsigned int)(((fluid_real_t)voice->output_rate * 0.001f * ms) /
                             (fluid_real_t)FLUID_BUFSIZE + 0.5f);

  if (countinc > 0) {
    voice->portamento_active = 1;
    voice->portamento_pitch_offset = pitchoffset;
    voice->portamento_pitch_incr = pitchoffset / (countinc * (fluid_real_t)FLUID_BUFSIZE);
  }
}
...
fluid_voice_calculate_gen_pitch(fluid_voice_t* voice)
{
  ...
  if (fluid_channel_has_tuning(voice->channel)) {
    ...
    fluid_tuning_t* tuning = fluid_channel_get_tuning(voice->channel);
    x = __pitch((int) (voice->root_key / 100.0f));
    voice->gen[GEN_PITCH].val = (x + (voice->gen[GEN_SCALETUNE].val / 100.0f *
                                (__pitch(voice->key) - x)));
  } else {
    voice->gen[GEN_PITCH].val = (voice->gen[GEN_SCALETUNE].val * (voice->key - voice->root_key / 100.0f)
                                + voice->root_key);
  }
}
...
fluid_voice_update_param(fluid_voice_t* voice, int gen)
{
  ...
  case GEN_OVERRIDEROOTKEY:
     ...
     */
    {
      fluid_real_t z = voice->sample != NULL ? voice->sample->samplerate : voice->output_rate;
      fluid_real_t x;
...
int fluid_voice_get_channel_num(fluid_voice_t* voice)
{
  return voice->chan;
}
...
{
    /* Early bail-out: if the sample or its data is NULL, turn off the voice */
    if (voice->sample == NULL || voice->sample->data == NULL) {
      FLUID_LOG(FLUID_WARN, "voice %p: sample or sample->data is NULL, turning off", (void*)voice);
      fluid_voice_off(voice);
      return;
    }

    /* If sample bounds are inverted, the sample is invalid */
    if (voice->sample->start > voice->sample->end) {
      FLUID_LOG(FLUID_WARN, "voice %p: invalid sample bounds start=%d end=%d, turning off",
                (void*)voice, (int)voice->sample->start, (int)voice->sample->end);
      fluid_voice_off(voice);
      return;
