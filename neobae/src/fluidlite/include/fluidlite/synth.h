...
FLUIDSYNTH_API int fluid_synth_noteoff(fluid_synth_t* synth, int chan, int key);
  ...
  /** Immediately stop all sounding notes on a channel (MIDI all sounds off). */
FLUIDSYNTH_API int fluid_synth_all_sounds_off(fluid_synth_t* synth, int chan);

  /** Send an all-notes-off message to a channel (MIDI all notes off). */
...
FLUIDSYNTH_API fluid_preset_t* fluid_synth_get_channel_preset(fluid_synth_t* synth, int chan);
  ...
  /** Unset (clear) the preset on a channel. The channel will have no
      preset assigned after this call. */
FLUIDSYNTH_API int fluid_synth_unset_program(fluid_synth_t* synth, int chan);

  /** Set the channel type (drum or melodic). In fluidlite, drum channels
      are identified by bank number (128). This function is provided for API
      compatibility and is a no-op. */
#define CHANNEL_TYPE_MELODIC 0
#define CHANNEL_TYPE_DRUM    1
FLUIDSYNTH_API void fluid_synth_set_channel_type(fluid_synth_t* synth, int chan, int type);

  /** Enable/disable reverb on a channel group (-1 for all). */
FLUIDSYNTH_API void fluid_synth_reverb_on(fluid_synth_t* synth, int fx_group, int on);

  /** Enable/disable chorus on a channel group (-1 for all). */
FLUIDSYNTH_API void fluid_synth_chorus_on(fluid_synth_t* synth, int fx_group, int on);
